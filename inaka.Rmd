...
scrptrvw <- scrptrvw[sample(1:nrow(scrptrvw)),]
...
reactable(
  ...
  columns = list(
    Kanji = colDef(
      # Show species under character names
      cell = function(value, index) {
        Meaning <- scrptrvw$Meaning[index]
        Meaning <- if (!is.na(Meaning)) Meaning else ""
        NR <- scrptrvw$NR[index]
        NR <- if (!is.na(NR)) NR else ""
        KR <- scrptrvw$KR[index]
        KR <- if (!is.na(KR)) KR else ""
        tagList(
          div(style = list(fontSize = 50, fontWeight = 600, fontFamily = 'Open Sans'), value),
          # div(style = list(fontSize = 9, color = '#008000'), NR),
          # div(style = list(fontSize = 9, color = '#800000'), KR),
          # div(style = list(fontSize = 9, fontWeight = 500), Meaning)
        )
...
reactable(
  scrptrvw.b,
  columns = list(
    Word = colDef(align = "center", style = list(fontSize = 28, fontWeight = 600, fontFamily = 'Open Sans'))
  ),
  defaultPageSize = 1,
  theme = reactableTheme(
    # Vertically center cells
...
spclkjn.a <- spclkjn %>% select(Kanji)
...
reactable(
  ...
  theme = reactableTheme(
    # Vertically center cells
    cellStyle = list(display = "flex", flexDirection = "column", justifyContent = "center", height = 307),
    searchInputStyle = list(
      paddingLeft = "8px",
      paddingTop = "8px",
      paddingBottom = "8px",
...
reactable(
  ...
  showPageInfo = FALSE, bordered = TRUE, searchable = TRUE, sortable = FALSE, paginationType = "numbers", height = 660, resizable = TRUE,
...
reactable(
  ...
  showPageInfo = FALSE, bordered = TRUE, searchable = TRUE, sortable = FALSE, paginationType = "numbers", height = 350, resizable = TRUE,
