...
adjimre <- adjimre[sample(1:nrow(adjimre)),]
...
reactable(
  ...
  columns = list(
    Translation = colDef(
      cell = function(value, index) {
        Sentences <- adjimre$Sentences[index]
        Sentences <- if (!is.na(Sentences)) Sentences else ""
        tagList(
          div(style = list(fontSize = 21, fontWeight = 600, color = '#404040'), Sentences),
        )
...
reactable(
  ...
  columns = list(
    Sentences = colDef(
      cell = function(value, index) {
        Translation <- adjimre$Translation[index]
        Translation <- if (!is.na(Translation)) Translation else ""
        tagList(
          div(style = list(fontSize = 12, fontWeight = 600, color = '#404040'), Translation),
        )
...
nounw.a <- nounw %>% slice(1:660) %>% rename(Sentences = S1)
...
nounw.a <- nounw %>% slice(1:517) %>% rename(Sentences = S1, Translation = T1)
