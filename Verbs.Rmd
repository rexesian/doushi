...
<h6>**Language Learning Concept**</h6>
...
```
<!-- <h6>**Learning Japanese Concept**</h6> -->

<!-- ```{r, echo=FALSE, message=FALSE} -->

<!-- memb <- mem %>% slice(91:103) -->
...
<h6>**Strategy for learning Japanese **</h6>

```{r, echo=FALSE, message=FALSE}

memb <- mem %>% slice(125:139)
...
reactable(
  ...
  showPageInfo = FALSE, bordered = TRUE, searchable = FALSE, sortable = FALSE, paginationType = "jump", fullWidth = FALSE, width = 300, 
...
Introduction of **IC - Initial Carriage**, **MC - Main Carriage** and **E - Engine**.  
Copula - da (plain form)・desu (polite form)・dearu (literary form).

**①　Verbal Sentence**
`Verb as the engine`

###### **MC describes the action・state of E**
...
###### **MC defines・identifies・describes E**
...
`Verb aru・iru as the engine`

###### **E expresses the existence of MC**

    ・MC is E
    ・ここに車がある
    ・Koko ni kuruma ga aru
    ・There is a car here

**⑥　Topic-comment Sentence**
`Initial carriage as the topic, Main carriage and Engine as the comment`

###### **IC denotes the general talking point describe by MC and E**
    ...
    Expression = colDef(
      ...
      cell = function(value, index) {
        ...
        tagList(
          ...
          div(style = list(fontSize = 2), "."),
          # div(style = list(fontSize = 1), "."),
          div(style = list(fontSize = 10, fontWeight = 500), Ext, "▫︎" , Approx),
          div(style = list(fontSize = 10), Remarks),
        )
      },
      align = "center"
...
reactable(
  dctb,
  columns = list(
    Words = colDef(style = list(fontSize = 13, fontWeight = 400), align = "left"),
...
<p style="font-size:20px; line-height: 1.4; font-family: Yomogi; color: #202020">
干物は、丸みを帯びた形のもの、自然なツヤのあるものを選ぶ
新鮮で脂が乗っていて肉の厚みのあるもの。これはどんな種類の魚にも共通するおいしい干物の見分け方だ。
色は魚の種類によって少し異なるが、ツヤのいいものを選びたい。
形では、開きにしたとき側面が丸みを持っているかどうかで、材料の魚のよしあしを見極める。
やせた魚を開くと腹が薄いため細長い印象になり、身が厚いものほど円形に近い形に見えるのだ。
色はアジやイワシのような青魚の場合、赤みがかったベッコウ色、カマスに代表される白身魚は白みがかったアメ色で、透明感のあるツヤを持つものがおいしい。
鮮度を見るには、においをかいでみるのがいちばんだが、脂焼けして黄色がかったり鉄サビのような色になっているものは、できてから時間がたったものと考えていい。
</p>

---

<p style="font-size:12px; font-family: Klee One;">
Dried fish should be rounded in shape and have a natural luster.
It should be fresh, fatty, and have thick meat. This is the same way to recognize good dried fish of any kind. 
The color varies slightly depending on the type of fish, but you want to choose the ones with a good luster.
In terms of shape, you can tell if the fish is good or not by the roundness of the sides when it is opened.
A skinny fish with a thin belly will give the impression of being long and thin, while a thicker fish will look more circular.
Blue fish such as horse mackerels and sardines are reddish-bekko, while white fish such as barracuda are a white amber color with a translucent sheen.
The best way to check freshness is to smell the fish. If the fish is yellowish or has a rusty iron color due to fat burning, you can assume that it has been fresh for some time.
</p>
...
<p style="font-size:20px; line-height: 1.4; font-family: Yomogi; color: #202020">
コーヒーはよく売れている店で、一～二週間で飲みきれる量を買う
コーヒーを量り売りで買うとき、おいしい店を選ぶには、まず、よく売れている店を選ぼう。店頭の豆を見て、炒りムラがなくて、粒がそろっていれば、いいコーヒーだと思っていいだろう。
豆は、自分の好みの銘柄があれば、それを買えばいいし、とくになければ、好みを伝えて選んでもらうとよい。
買う量は、豆の場合は一～二週間で飲みきれる量を買うこと。ひとり一回分の使用量の目安は八～一〇グラム。新しいうちに飲みきるのが、コーヒーをおいしく飲むヒケツだ。
とくに、豆を挽いてもらうときは、一週間～十日で飲みきる量にする。抽出器具によって挽き方も変わるから、どんな抽出器具を使うのか、ちゃんと伝えよう。
</p>

---

<p style="font-size:12px; font-family: Klee One;">
Buy coffee in quantities that you can drink in a week or two at stores that sell well.
When buying coffee by weight, the first step in choosing a good coffee shop is to choose one that sells well. If the beans are evenly roasted and the grain is uniform, it is safe to assume that the coffee is good.
If there is a brand of beans that you prefer, buy it. If there is no particular brand, tell the shop what you like and ask them to choose for you.
If there is no particular brand, you can let them know your preferences and ask them to select one for you. The standard amount for one serving for one person is 8 to 10 grams. The key to good coffee is to drink it while it is still fresh.
...
<p style="font-size:20px; line-height: 1.4; font-family: Yomogi; color: #202020">
海で泳ぐときは、浜辺と平行に泳ぐと安全
海水浴場では、波打ち際は混雑していて、ゆっくり泳げないが、沖に向かってどんどん泳いでいって、はっと気がついたら岸から離れすぎていた…なんていうのは危険だ。
そこで、波打ち際の混雑を抜けたところで、浜辺と平行に泳ぐといい。広々したところを長い距離泳げて、岸から離れすぎてしまう心配もない。「監視台からあの岩まで」など、区間を決めて泳ぐといいだろう。
オレンジ色や黄色などの目立つ色のキャップをかぶると、もっと安全だ。
もし、遠泳できる力をつけたいと思ったら、本当に遠くまで泳いでいかなくても、二十分とか三十分と、時間を決めてトレーニングすると効果がある。
</p>

---

<p style="font-size:12px; font-family: Klee One;">
When swimming in the ocean, it is safer to swim parallel to the beach.
At the beach, the surf is so crowded that it is difficult to swim slowly, but it is dangerous to swim out to sea and then realize that you are too far from the shore ....
It is better to swim parallel to the beach after the surf, when you get out of the crowd. You can swim a long distance in a wide open area and not worry about being too far from the shore.
It is best to swim in sections, such as "from the watchtower to that rock.
Wearing a cap of a conspicuous color, such as orange or yellow, will make it safer.
If you want to improve your ability to swim long distances, you don't have to swim really far, but training for 20 or 30 minutes at a time can be effective.
</p>
...
<p style="font-size:20px; line-height: 1.4; font-family: Yomogi; color: #202020">
わかりやすい略地図を書くコツは、進行方向を上にすること
地図は、ふつう、北を上に書くものだが、人に道順を教えるときに書く略地図は、その地図を見ながら歩く人のことを考え、進行方向を上にするとわかりやすい。
電車で来る場合は、最寄り駅から。また、バスターミナルのあるような大きな駅をスタート地点として書く場合は、出口がたくさんあり、目的の出口にたどり着くのもたいへんなので、地下鉄の出口番号とか、東口とか西口といった出口名を入れておくとよい。
駅近くや途中の道筋に、銀行、デパート、ファーストフード店など、すぐにわかるような目立つ建物があれば、それらも書き込み、歩く方向の矢印も入れ、それらを目標としながら歩けるようにすると、わかりやすい。
</p>

---

<p style="font-size:12px; font-family: Klee One;">
The key to drawing an easy-to-understand abbreviated map is to put the direction of travel on top.
Maps are usually drawn with the north side up, but when drawing an abbreviated map to give directions to others, it is easier to understand if the direction of travel is placed on top, considering the people who will be walking while looking at the map.
If coming by train, start from the nearest station. If the starting point is a large station with a bus terminal, there are many exits and it is difficult to reach the desired exit, so it is better to include the exit number of the subway or the name of the exit, such as East Exit or West Exit.
If there are any prominent buildings near the station or along the way, such as banks, department stores, fast food restaurants, etc., write them down and include an arrow pointing in the direction you will be walking so that you can walk while keeping them as your goal.
</p>
