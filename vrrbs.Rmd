...
verbrsent <- verbrw %>% select(S1, T1)
verbrsent <- verbrsent %>% rename(Sentences.NH = S1, Sentences = T1, )
verbrsent <- verbrsent %>% na.omit(verbrsent)
verbrsent <- verbrsent[sample(1:nrow(verbrsent)),]
reactable(
  verbrsent,
  columns = list(
    Sentences = colDef(
      cell = function(value, index) {
        Sentences.NH <- verbrsent$Sentences.NH[index]
        Sentences.NH <- if (!is.na(Sentences.NH)) Sentences.NH else ""
        tagList(
          div(style = list(fontSize = 14, fontWeight = 600), Sentences.NH),
    ...
    Sentences.NH = colDef(show = FALSE)
